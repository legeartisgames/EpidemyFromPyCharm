...
class Country_choiser_layout(FloatLayout):
    ...
    def update_country_menu():
        ...
        for i in country_variants.numbers_in_menu[common_var.lang]:
            ...
            if common_data.my_stats.stars >= i.min_rep_stars and common_data.my_stats.goldreserves >= i.min_goldreserves:
                common_var.num_of_lands+=1
                aval_state_countries.append(True)
            else:
                if common_data.my_stats.stars < i.min_rep_stars:
                    aval_state_countries.append([False, 'stars', - common_data.my_stats.stars + i.min_rep_stars])
                else:
                    aval_state_countries.append([False, 'coins', - common_data.my_stats.goldreserves + i.min_goldreserves])
    ...
    def open_layout(self, instance):
        ...
        self.add_widget(uix_classes.Label_with_tr(text_source=["Выберите страну, за которую будете играть:", "Choose the country to play with:"], 
                                                  ...
                                                  font_size = (sizes.Width_of_screen/53)))
        ...
        self.btn_remake_country = uix_classes.Button_with_image(text_source = ["Подтвердить выбор", "Confirm choice"], size_hint_x = None, width = sizes.Width_of_screen/4.5, size_hint_y = .15, 
        ...
        self.btn_country_info = uix_classes.Button_with_image(text_source=frases.str_open_country_info, size_hint_x = None, size_hint_y = .15, width = sizes.Width_of_screen / 4.5, 
