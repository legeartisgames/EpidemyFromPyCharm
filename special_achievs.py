    ...
    def __init__(self, achieves=[], color="gold", **kwargs):
        ...
        for i in range(len(achieves)):
            ...
            if achieves[i][0] == "rate_app":
                self.text += ['Не забудьте написать отзыв на [u][ref=' + achieves[i][
                    1] + ']Google Play[/ref][/u]\nи поставить оценку приложению (желательно 5 звёзд :) )!\n\nЭто будет Ваша лучшая помошь "Эпидемии".\n',
                              "Don't forget to rate our app in [u][ref=" + achieves[i][
                                  1] + "]Google Play[/ref][/u]!\n\n"][common_var.lang]
            ...
            if achieves[i][0] == "new_tech":
                self.text += (["Разблокирован новый метод: ",
                               ...
                               ][common_var.lang] + "[b][color=ff0066]" + tech_info.names_of_tech[achieves[i][1]][
                                  common_var.lang] + "[/color][/b]\n\n")

            if achieves[i][0] == "new_disease":
                self.text += (["Разблокирована новая инфекция: ",
                               "New infection is unblocked: ",
                               ][common_var.lang] + "[b][color=ed1313]" +
                              diseases.diseases_list[achieves[i][1][0]].name[common_var.lang] + "[/color][/b]\n\n")
            ...
            if achieves[i][0] == "new_techs":
                if len(achieves[i][1]) == 1:
                    ...
                    self.text += (["Разблокирован новый метод: ",
                                   "New method is unblocked: ",
                                   ][common_var.lang] + "[b][color=ff0066]" +
                                  tech_info.names_of_tech[achieves[i][1][0]][common_var.lang] + "[/color][/b]\n\n")
                else:
            ...
            if achieves[i][0] == "new_countries":
                if len(achieves[i][1]) == 1:
                    ...
                    self.text += (["Разблокирована новая страна: ",
                                   ...
                                   ][common_var.lang] + "[b][color=" + country_variants.Country_lands[
                                      achieves[i][1][0]].color_of_name + "]" +
                                  country_variants.Country_lands[achieves[i][1][0]].name[
                                      common_var.lang] + "[/color][/b]\n\n")
                else:
                    add = ["Разблокированы новые страны: ", "New countries are unblocked: "][common_var.lang]
                    for k in range(len(achieves[i][1])):
                        add += "[b][color=" + country_variants.Country_lands[achieves[i][1][k]].color_of_name + "]" + \
                               country_variants.Country_lands[achieves[i][1][k]].name[common_var.lang]
                        if k == len(achieves[i][1]) - 1:
                            add += ".[/b][/color]\n\n"
                        else:
                            add += ',[/b][/color] '
            ...
            if achieves[i][0] == "rep_status":
                self.text += (["У Вас новый статус репутации: ",
                               "You've got new reputation status: "][common_var.lang] + "[b][color=14BADB]" + str(
                    common_var.stars_statuses[achieves[i][1]][common_var.lang]) + '\n\n[/b][/color]')
            if achieves[i][0] == "gold_status":
                self.text += (["У Вас новый статус благостояния: ",
                               "You've got new wealth status: "][common_var.lang] + "[b][color=fbc503]" + str(
                    common_var.statuses_coins[achieves[i][1]][common_var.lang]) + '\n\n[/b][/color]')
...
class SpecAchievPanel(cd.FloatLayout):
    def __init__(self, achieves=[], color="gold", **kwargs):
        super(SpecAchievPanel, self).__init__(**kwargs)
        with self.canvas:
            Color(.98, .77, .01)
            if color == "green":
                Color(22 / 256, 186 / 256, 91 / 256)
            Line(
                rectangle=(.03 * sizes.width_res, .05 * sizes.height_res, .94 * sizes.width_res, .9 * sizes.height_res),
                width=sizes.height_res / 720 * 3)

        self.bind(on_touch_down=self.touched_to_close)

        self.bl = BoxLayout(size_hint=[.7, .2], pos_hint={'center_x': .5, 'center_y': .17},
                            orientation="horizontal", spacing=sizes.width_res / 50)
        ...
        for i in achieves:
            if i[0] == "new_techs":
                for j in range(len(i[1])):
                    im2 = Widget_with_texture(size_hint_y=None, height=self.bl.size_hint_y * sizes.height_res,
                                              size_hint_x=None, width=self.bl.size_hint_y * sizes.height_res * 8 / 10)
            ...
            if i[0] == "new_countries":
                ...
                for j in range(len(i[1])):
                    im1 = Widget_with_texture(size_hint_y=None, height=self.bl.size_hint_y * sizes.height_res,
                                              size_hint_x=None, width=self.bl.size_hint_y * sizes.height_res * 1.6)
                    ...
                    with im1.canvas:
                        ...
                        Color(1, 1, 1, .8)
                        im1.rect = Rectangle(size=im1.size, pos=im1.pos, texture=textures.CoreImage(
                            "flags_images/" + cd.country_variants.flags_source[i[1][j]] + ".jpg").texture)
                        im1.bind(size=im1.update_rect, pos=im1.update_rect)
        ...
        self.bl.size_hint_x = counter / sizes.width_res + 0
        self.bl.pos_hint['center_x'] = .5

        cd.random.shuffle(im)
        for i in im:
