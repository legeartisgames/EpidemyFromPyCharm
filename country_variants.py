NUM_OF_COUNTRIES = 13
class Land:
    def __init__(self, index, Cart_of_country, Number_of_regions, Capital_index, dens, pop, income_of_c, size, dy, la_z_ins, stars, level, name,
                 min_reputation_status, min_coins_status, fluct_z_out, ups_by_y, real_size_of_hex, color_of_name, half_ball, coef_perenos,
                 crit_b, pp_name, names_of_provinces):
        self.index = index
...
India_cart[9]=[3,0]
capital_of_India = 3
India_names = [["Ин Обл.","InRegion"]]*number_of_regions_in_India
...
Mexico_names = [["Ме Обл.", "MeRegion"]]*number_of_regions_in_Mexica
...
Russia_names = [["Ру Обл.", "RuRegion"]]*number_of_regions_in_Russia
...
Turkey_names = [["Ту обл", "TuRegion"]]*number_of_regions_in_Turkey
...
USA_names = [["США обл.", "USRegion"]]*number_of_regions_in_USA
...
Universal_names = [["Ун обл.", "UnRegion"]]*7
...
for i in range(13):
    Country_lands[i] = Land(index = i,
                            ...
                            half_ball = half_balls[i],
                            coef_perenos = country_coefs_base[i],
                            crit_b = crit_straph_b[i],
                            pp_name = pp_names[i],
